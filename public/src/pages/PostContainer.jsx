...
import {getPosts, upvote, addComment, addContact, report} from "../utils/APIRoutes";
        ...
        try{
          if(!currentUser) return;
            ...
            if(response.status === 200){
                ...
                const filteredPosts = response.data.filter((post) => !post.reported.includes(currentUser._id));
      ...
      return(
        <Container key={post._id}>
          <div className='post-header'>
            ...
            {/* <div className='chat-btn' onClick = {() => setContact(post.userId)}>
              <span>chat</span>
            </div> */}
            <div className={`chat-btn ${currentUserId === post.userId ? 'disabled' : ''}`} onClick={() => setContact(post.userId)}>
                <span>chat</span>
            </div>
